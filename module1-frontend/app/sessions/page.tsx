                  ...
                  <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-3">
                        <h3 className="text-lg font-semibold text-slate-950">{[session.courseCode, session.courseName].filter(Boolean).join(' ') || 'Course unavailable'}</h3>
                      ...
                      <p className="mt-1 text-sm text-slate-500">{session.sessionName}</p>
                      <dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-4">
                        <div><dt className="text-slate-500">Session type</dt><dd className="mt-1 font-medium text-slate-900">{formatSessionType(session.sessionType)}</dd></div>
                        <div><dt className="text-slate-500">Location</dt><dd className="mt-1 font-medium text-slate-900">{session.venueName ?? 'Not specified'}</dd></div>
                        ...
                        <div><dt className="text-slate-500">Check-in closes</dt><dd className="mt-1 whitespace-nowrap font-medium text-slate-900">{formatDateTime(session.checkinClosesAt)}</dd></div>
                      </dl>
                      <div className="mt-5 flex flex-wrap gap-2" aria-label="Verification requirements">
                        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700">Location required</span>
                        {session.requireLivenessCheck && <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700">Liveness check</span>}
                        {session.requireFaceMatch && <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700">Face verification</span>}
                        {session.qrCodeEnabled && <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700">QR code</span>}
                      </div>
                    </div>
