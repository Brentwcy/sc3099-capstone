...
def _build_aesgcm(key_b64: str) -> "_AESGCM | None":
    ...
    try:
        ...
        logger.warning("Failed to load TEMPLATE_ENCRYPTION_KEY")
        return None
    ...
    def set_template(self, hash_key: str, simhash: str, ttl_seconds: Optional[int] = None) -> bool:
        ...
        if self.redis_client:
            try:
                ...
                logger.debug("Redis template set failed, using memory fallback")
    ...
    def delete_template(self, hash_key: str) -> bool:
        ...
        if self.redis_client:
            try:
                ...
                logger.warning("Redis template delete failed")
