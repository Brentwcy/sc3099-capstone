...
|---|---|---|---|
| Backend focused | `cd module2-backend && PYTHONPATH=. .venv/bin/pytest tests -q` | 43 passed in 28.98s on 2026-09-03 | Maanya; peer review pending |
| Dashboard focused | `cd module4-observability && python3 -m pytest tests -q` | 9 passed | Maanya |
| Backend compile | `cd module2-backend && python3 -m compileall -q app tests` | Passed on 2026-08-27 | Maanya |
...
| Alembic lifecycle | Fresh SQLite upgrade from base through `20260827_0006`; `alembic current`; `alembic heads` | Risk-signals migration applied and single head confirmed on 2026-08-27; PostgreSQL rehearsal still required | Maanya |
...
| Week 5 backend acceptance | Device lifecycle/RBAC/reuse, filtered check-in query, attempt/outcome audits, risk persistence, session attendance summary, Module 3 client contract/timeouts/errors | 17 focused tests passed on 2026-09-03 | Maanya; Module 3 contract and backend peer review pending |
